...
"+-----------------------------------------------------------------------------+" << '\n';
  ...
  void InputOutputColor::Redraw() {
    ...
    out << ColorField("32","+-----------------------------------------------------------------------------+")
        << '\n' << ColorField("32","|")
        << ColorField("2;36","                                   CodeMines                                 ")
        << ColorField("32","|") << '\n' << ColorField("32","|")
        << ColorField("2;34","                                By Peter Schultz                             ")
        << ColorField("32","|") << '\n'
        << ColorField("32","+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-------------------+")
        << '\n' << ColorField("32","|") << ColorField("2;31",lsmiley);
    for (tile_t i = 0; i < NUMBER_OF_COLS; ++i) {
      char letter[2] = {horizBar[i], 0};
      out << ColorField("32","|") << ColorField("2;34", letter);
    }
    out << ColorField("32","|") << ColorField("2;31",rsmiley)
        << ColorField("32","|") << ColorField("2;36","     Commands:     ")
        << ColorField("32","|") << '\n';
    for (std::size_t i = 0; i < matrix.size(); ++i) {
      out << ColorField("32","|") << ColorField("2;34",numbers[i])
          << ColorField("32","|");
      for (std::size_t j = 0; j < matrix[i].size(); ++j) {
        ...
        if (userIsCheating == 2 && (Tile::State(matrix[i][j]) == Tile::Volatile)) {
          ...
          switch (tile[0]) {
            case Tile::Numeral_1: { color = "2;36"; } break;
            case Tile::Numeral_2: { color = "2;31"; } break;
            case Tile::Numeral_3: { color = "2;32"; } break;
            case Tile::Numeral_4: { color = "2;35"; } break;
            case Tile::Numeral_5: { color = "36"; } break;
            case Tile::Numeral_6: { color = "31"; } break;
            case Tile::Numeral_7: { color = "32"; } break;
            case Tile::Numeral_8: { color = "35"; } break;
            case Tile::Flag: { color = "2;34"; } break;
            case Tile::Unknown: { color = "36"; } break;
            case Tile::TrippedMine:
            case Tile::Mine: { color = "2;41"; } break;
            default: {  } break;
          }
      ...
      out << ColorField("32","|") << ColorField("2;34",numbers[i])
          << ColorField("32","|") << ColorField("2;34",rightSideBar[i])
          << ColorField("32","|") << '\n';
    }
    out << ColorField("32","|") << ColorField("2;31",lsmiley);
    for (tile_t i = 0; i < NUMBER_OF_COLS; ++i) {
      char letter[2] = {horizBar[i], 0};
      out << ColorField("32","|") << ColorField("2;34", letter);
    }
    out << ColorField("32","|") << ColorField("2;31",rsmiley)
        << ColorField("32","|") << ColorField("2;36","    Mines: ")
        << std::setw(3) << std::left << logical_cast<std::string>(engine.Mines())
        << std::right << std::setw(5) << ' ' << ColorField("32","|") << '\n'
        << ColorField("32","+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-------------------+")
        << '\n' << ColorField("32","|") << ColorField("2;31"," {{ ")
        << lastMessage << ColorField("2;31"," }} ")
        << std::setw(69 - lastMessage.size()) << ' ' << ColorField("32","|")
        << '\n' << ColorField("32","+") << ColorField("1;34"," >> ")
        << std::flush;
  }
